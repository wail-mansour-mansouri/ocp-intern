...
\paragraph{Résoudre.} Le modèle atteint l'optimum global en moins de \SI{0.1}{\second} et satisfait
...
Quatre résultats dépassent le cadre strict du livrable attendu.
...
\subsection{La mise en évidence d'une contrainte de configuration}

Le quatrième apport n'était pas prévu : il découle des trois premiers. En modélisant
fidèlement le procédé, puis en analysant la solution, on découvre que les seules violations
résiduelles proviennent d'un \emph{paramètre} du cahier des charges --- l'affectation des
échelons à la cocristallisation --- et non d'une mauvaise décision d'exploitation.

Ce type de résultat est le plus difficile à obtenir : il suppose d'avoir d'abord respecté la
spécification assez scrupuleusement pour que le modèle soit crédible, puis d'accepter de la
remettre en question. Un modèle qui aurait dès le départ traité ce paramètre comme une
variable n'aurait rien révélé du tout --- il aurait simplement produit une bonne solution,
sans jamais signaler que la contrainte existait.
...
Cette limite se manifeste concrètement dans le mode décidable de la
section~\ref{sec:mode-decidable} : le modèle réduit fortement la cocristallisation pour
vider un bac trop rempli, sans se soucier de le reconstituer. Le chiffre obtenu mesure un
potentiel, pas une consigne.
...
\subsection{Instruire la question de la cocristallisation}

Cette piste a été explorée et chiffrée (section~\ref{sec:mode-decidable}) : rendre
l'affectation des échelons décidable résorbe intégralement les deux violations. Ce qui reste
à faire ne relève plus de la modélisation mais de l'exploitation --- il faut vérifier qu'une
telle reconfiguration est opérationnellement possible, et à quelle fréquence les unités de
cocristallisation peuvent être démarrées et arrêtées sans dommage.

C'est le seul point de ce rapport où le modèle propose une action que le cahier des charges
excluait par construction. Il est donc soumis au service sous forme de question, non de
recommandation.
