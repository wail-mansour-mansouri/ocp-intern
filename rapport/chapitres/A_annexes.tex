...
\paragraph{Variante D-11 --- cocristallisation décidable.}
Quand le mode optionnel est activé, chaque échelon raccordé à une unité de cocristallisation
reçoit une binaire $z_e$ et l'on substitue partout
\begin{equation}
\Picoc_m = \sum_{e \in \Ech^{\mathrm{coc}}_m} \kappa_e\, z_e
\end{equation}
La contrainte C5 devient alors $\xdec_\ell \le \Pi_m - \Picoc_m$ : éteindre un échelon
cocristallisant libère de la capacité pour l'acide décadmié. Toutes les autres contraintes
restent inchangées dans leur écriture, et toutes restent affines.
...
\begin{encadre}
Le solde net journalier d'IR11 --- entrées \SI{2431,3}{\tonne} moins sorties
...
\texttt{tests/} & 162 tests automatisés \\
...
\begin{table}[H]
\centering
\caption{Les onze décisions de modélisation}
\label{tab:decisions}
